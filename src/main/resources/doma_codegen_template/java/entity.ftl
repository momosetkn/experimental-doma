...
public record <#if entityPrefix??>${entityPrefix}</#if>${simpleName}<#if entitySuffix??>${entitySuffix}</#if>(
...
<#if property.showColumnName && property.columnName??>
    ...
    ${property.propertyClassSimpleName} ${property.name}<#if property_has_next || originalStatesPropertyName??>,</#if>
</#list>
